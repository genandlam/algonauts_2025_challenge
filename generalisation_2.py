...
def model():
   ...
   # backend = set_backend("torch_cuda", on_error="warn")
    ...
    n_targets_batch = 1
    n_alphas_batch = 1
    n_targets_batch_refit = 1

    solver_params = dict(n_iter=n_iter, alphas=alphas,
                        n_targets_batch=n_targets_batch,
                        n_alphas_batch=n_alphas_batch,
                        n_targets_batch_refit=n_targets_batch_refit,
                        Ks_in_cpu= True,Y_in_cpu=True,
...
if __name__== "__main__":
    ...
    fmri_test = load_fmri(root_data_dir, 5)
    ...
    column_kernelizer = create_kernal()
    print("running model")
    model = model()
    pipe = make_pipeline(column_kernelizer, model)
    pipe.fit(features_train_all, fmri_train_all)
    with open('model_n_cv.pkl','wb') as f:
        pickle.dump(model,f)
    # Predict the fMRI responses for the validation set
    fmri_val_pred = pipe.predict(features_val)
    fmri_val_pred=fmri_val_pred.numpy()
    np.savetxt('val_n_cv.txt', fmri_val_pred)
    mean_acc_val=compute_encoding_accuracy(fmri_val, fmri_val_pred)
    with open("mean_acc_valn_cv.txt", "a") as f:
        f.write("Subject: " + str(3) + "\n")
        f.write("Modality: " + modality + "\n")
        f.write("Mean encoding accuracy: " + str(mean_acc_val) + "\n")
    print("Mean encoding accuracy:", mean_acc_val)
    print("Results saved to mean_acc_valn_cv.txt")
    # Predict the fMRI responses for the test set
    fmri_test_pred = pipe.predict(features_test)
    fmri_test_pred=fmri_test_pred.numpy()
    np.savetxt('test.txt', fmri_test_pred)
    mean_acc_test=compute_encoding_accuracy(fmri_test, fmri_test)
    with open("mean_acc_test_n_cv.txt", "a") as f:
        f.write("Subject: " + str(5) + "\n")
        f.write("Modality: " + modality + "\n")
        f.write("Mean encoding accuracy: " + str(mean_acc_test) + "\n")
    print("Mean encoding accuracy:", mean_acc_test)
    print("Results saved to mean_acc_valn_cv.txt")
